...
_logLanguageRegex = {
    'english': {
        ...
        'pilotAndWeapon':
        '(?:.*ffffffff>(?P<default_pilot>[^\(\)<>]*)(?:\[.*\((?P<default_ship>.*)\)<|<)/b.*> \-(?: (?P<default_weapon>.*?)(?: \-|<)|.*))',
        'damageOut': "\(combat\) <.*?><b>([0-9]+).*>to<",
        'damageIn': "\(combat\) <.*?><b>([0-9]+).*>from<",
        'armorRepairedOut':
        "\(combat\) <.*?><b>([0-9]+).*> remote armor repaired to <",
        'hullRepairedOut':
        "\(combat\) <.*?><b>([0-9]+).*> remote hull repaired to <",
        'shieldBoostedOut':
        "\(combat\) <.*?><b>([0-9]+).*> remote shield boosted to <",
        'armorRepairedIn':
        "\(combat\) <.*?><b>([0-9]+).*> remote armor repaired by <",
        'hullRepairedIn':
        "\(combat\) <.*?><b>([0-9]+).*> remote hull repaired by <",
        'shieldBoostedIn':
        "\(combat\) <.*?><b>([0-9]+).*> remote shield boosted by <",
        'capTransferedOut':
        "\(combat\) <.*?><b>([0-9]+).*> remote capacitor transmitted to <",
        'capNeutralizedOut':
        "\(combat\) <.*?ff7fffff><b>([0-9]+).*> energy neutralized <",
        'nosRecieved':
        "\(combat\) <.*?><b>\+([0-9]+).*> energy drained from <",
        'capTransferedIn':
        "\(combat\) <.*?><b>([0-9]+).*> remote capacitor transmitted by <",
        'capNeutralizedIn':
        "\(combat\) <.*?ffe57f7f><b>([0-9]+).*> energy neutralized <",
        'nosTaken': "\(combat\) <.*?><b>\-([0-9]+).*> energy drained to <",
        'mined':
        "\(mining\) .* <b><.*?><.*?>([0-9]+).*> units of .*<b>(.+)</b>"
    },
    'russian': {
        'character':
        "(?<=Слушатель: ).*",
        ...
        "(?<=Сеанс начат: ).*",
        'pilotAndWeapon':
        '(?:.*ffffffff>(?:<localized .*?>)?(?P<default_pilot>[^\(\)<>]*)(?:\[.*\((?:<localized .*?>)?(?P<default_ship>.*)\)<|<)/b.*> \-(?: (?:<localized .*?>)?(?P<default_weapon>.*?)(?: \-|<)|.*))',
        'damageOut':
        "\(combat\) <.*?><b>([0-9]+).*>на<",
        'damageIn':
        "\(combat\) <.*?><b>([0-9]+).*>из<",
        'armorRepairedOut':
        "\(combat\) <.*?><b>([0-9]+).*> единиц запаса прочности брони отремонтировано <",
        'hullRepairedOut':
        "\(combat\) <.*?><b>([0-9]+).*> единиц запаса прочности корпуса отремонтировано <",
        'shieldBoostedOut':
        "\(combat\) <.*?><b>([0-9]+).*> единиц запаса прочности щитов накачано <",
        'armorRepairedIn':
        "\(combat\) <.*?><b>([0-9]+).*> единиц запаса прочности брони получено дистанционным ремонтом от <",
        'hullRepairedIn':
        "\(combat\) <.*?><b>([0-9]+).*> единиц запаса прочности корпуса получено дистанционным ремонтом от <",
        'shieldBoostedIn':
        "\(combat\) <.*?><b>([0-9]+).*> единиц запаса прочности щитов получено накачкой от <",
        'capTransferedOut':
        "\(combat\) <.*?><b>([0-9]+).*> единиц запаса энергии накопителя отправлено в <",
        'capNeutralizedOut':
        "\(combat\) <.*?ff7fffff><b>([0-9]+).*> энергии нейтрализовано <",
        'nosRecieved':
        "\(combat\) <.*?><b>\+([0-9]+).*> энергии извлечено из <",
        'capTransferedIn':
        "\(combat\) <.*?><b>([0-9]+).*> единиц запаса энергии накопителя получено от <",
        'capNeutralizedIn':
        "\(combat\) <.*?ffe57f7f><b>([0-9]+).*> энергии нейтрализовано <",
        'nosTaken':
        "\(combat\) <.*?><b>\-([0-9]+).*> энергии извлечено и передано <",
        'mined':
        "\(mining\) .* <b><.*?><.*?>([0-9]+).*<b>(?:<localized .*?>)?(.+)\*</b>"
    },
    'french': {
        'character':
        "(?<=Auditeur: ).*",
        ...
        "\(combat\) <.*?><b>([0-9]+).*>de<",
        'armorRepairedOut':
        "\(combat\) <.*?><b>([0-9]+).*> points de blindage transférés à distance à <",
        'hullRepairedOut':
        "\(combat\) <.*?><b>([0-9]+).*> points de structure transférés à distance à <",
        'shieldBoostedOut':
        "\(combat\) <.*?><b>([0-9]+).*> points de boucliers transférés à distance à <",
        'armorRepairedIn':
        "\(combat\) <.*?><b>([0-9]+).*> points de blindage réparés à distance par <",
        'hullRepairedIn':
        "\(combat\) <.*?><b>([0-9]+).*> points de structure réparés à distance par <",
        'shieldBoostedIn':
        "\(combat\) <.*?><b>([0-9]+).*> points de boucliers transférés à distance par <",
        'capTransferedOut':
        "\(combat\) <.*?><b>([0-9]+).*> points de capaciteur transférés à distance à <",
        'capNeutralizedOut':
        "\(combat\) <.*?ff7fffff><b>([0-9]+).*> d'énergie neutralisée en faveur de <",
        'nosRecieved':
        "\(combat\) <.*?><b>([0-9]+).*> d'énergie siphonnée aux dépens de <",
        'capTransferedIn':
        "\(combat\) <.*?><b>([0-9]+).*> points de capaciteur transférés à distance par <",
        'capNeutralizedIn':
        "\(combat\) <.*?ffe57f7f><b>([0-9]+).*> d'énergie neutralisée aux dépens de <",
        'nosTaken':
        "\(combat\) <.*?><b>([0-9]+).*> d'énergie siphonnée en faveur de <",
        'mined':
        "\(mining\) .* <b><.*?><.*?>([0-9]+).*<b>(?:<localized .*?>)?(.+)\*</b>"
    },
    'german': {
        ...
        "\(combat\) <.*?><b>([0-9]+).*>von<",
        'armorRepairedOut':
        "\(combat\) <.*?><b>([0-9]+).*> Panzerungs-Fernreparatur zu <",
        'hullRepairedOut':
        "\(combat\) <.*?><b>([0-9]+).*> Rumpf-Fernreparatur zu <",
        'shieldBoostedOut':
        "\(combat\) <.*?><b>([0-9]+).*> Schildfernbooster aktiviert zu <",
        'armorRepairedIn':
        "\(combat\) <.*?><b>([0-9]+).*> Panzerungs-Fernreparatur von <",
        'hullRepairedIn':
        "\(combat\) <.*?><b>([0-9]+).*> Rumpf-Fernreparatur von <",
        'shieldBoostedIn':
        "\(combat\) <.*?><b>([0-9]+).*> Schildfernbooster aktiviert von <",
        'capTransferedOut':
        "\(combat\) <.*?><b>([0-9]+).*> Fernenergiespeicher übertragen zu <",
        'capNeutralizedOut':
        "\(combat\) <.*?ff7fffff><b>([0-9]+).*> Energie neutralisiert <",
        'nosRecieved':
        "\(combat\) <.*?><b>\+([0-9]+).*> Energie transferiert von <",
        'capTransferedIn':
        "\(combat\) <.*?><b>([0-9]+).*> Fernenergiespeicher übertragen von <",
        'capNeutralizedIn':
        "\(combat\) <.*?ffe57f7f><b>\-([0-9]+).*> Energie neutralisiert <",
        'nosTaken':
        "\(combat\) <.*?><b>\-([0-9]+).*> Energie transferiert zu <",
        'mined':
        "\(mining\) .* <b><.*?><.*?>([0-9]+).*<b>(?:<localized .*?>)?(.+)\*</b>"
    },
    'japanese': {
        ...
        "\(combat\) <.*?><b>([0-9]+).*> remote capacitor transmitted to <",
        'capNeutralizedOut':
        "\(combat\) <.*?ff7fffff><b>([0-9]+).*> エネルギーニュートラライズ 対象:<",
        'nosRecieved':
        "\(combat\) <.*?><b>\+([0-9]+).*> エネルギードレイン 対象:<",
        'capTransferedIn':
        "\(combat\) <.*?><b>([0-9]+).*> remote capacitor transmitted by <",
        'capNeutralizedIn':
        "\(combat\) <.*?ffe57f7f><b>([0-9]+).*>のエネルギーが解放されました<",
        'nosTaken':
        "\(combat\) <.*?><b>\-([0-9]+).*> エネルギードレイン 攻撃者:<",
        'mined':
        "\(mining\) .* <b><.*?><.*?>([0-9]+).*<b>(?:<localized .*?>)?(.+)\*</b>"
    },
    'chinese': {
...
class CharacterDetector(FileSystemEventHandler):
    def __init__(self, mainWindow, characterMenu):
        ...
        if (platform.system() == "Windows"):
            ...
            self.path = oShell.SpecialFolders(
                "MyDocuments") + "\\EVE\\logs\\Gamelogs\\"
        else:
        ...
        try:
            ...
            fileList = sorted(os.listdir(self.path),
                              key=lambda file: os.stat(
                                  os.path.join(self.path, file)).st_mtime)
            ...
            if len(self.menuEntries) == 0:
                self.characterMenu.menu.add_command(
                    label=tr("No character logs detected for past 24 hours"),
                    state=tk.DISABLED)

            self.observer.schedule(self, self.path, recursive=False)
            self.observer.start()
        except FileNotFoundError:
            logging.error('EVE logs directory not found, path checked: ' +
                          self.path)
            messagebox.showerror(
                tr("Error"),
                tr("Can't find the EVE logs directory.  Do you have EVE installed?"
                   ) + "\n\n" + tr("Path checked: ") + self.path + "\n\n" +
                tr("PELD will continue to run, but will not track EVE data."))
            self.characterMenu.menu.add_command(
                label=tr("No EVE installation detected"), state=tk.DISABLED)

        self.characterMenu.menu.add_separator()
        from settings.overviewSettings import OverviewSettingsWindow
        self.characterMenu.menu.add_command(label=tr("Open overview settings"),
                                            command=OverviewSettingsWindow)
    ...
    def addLog(self, logPath):
        ...
        self.characterMenu.menu.insert_radiobutton(0,
                                                   label=character,
                                                   variable=self.selectedIndex,
                                                   value=len(self.menuEntries),
                                                   command=self.catchupLog)
    ...
    def playbackLog(self, logPath):
        try:
            ...
            self.mainWindow.addPlaybackFrame(
                self.playbackLogReader.startTimeLog,
                self.playbackLogReader.endTimeLog)
        except BadLogException:
            self.playbackLogReader = None
...
class BaseLogReader():
    ...
    def createOverviewRegex(self, overviewSettings):
        if overviewSettings:
            ...
            try:
                keyLambda = lambda e: safeGetIndex(
                    e[0], overviewSettings['shipLabelOrder'])
                sortedShipLabels = sorted(overviewSettings['shipLabels'],
                                          key=keyLambda)
                ...
                for shipLabel in sortedShipLabels[:]:
                    ...
                    elif shipLabel[1]['type'] in [
                            'alliance', 'corporation', 'ship name'
                    ]:
                        safePre = re.escape(shipLabel[1]['pre'])
                ...
                pilotAndWeaponRegex += '|' + _logLanguageRegex[
                    self.language]['pilotAndWeapon'] + ')?'
    ...
    def compileRegex(self):
        basicPilotAndWeaponRegex = _logLanguageRegex[
            self.language]['pilotAndWeapon']
        basicPilotAndWeaponRegex += '(?P<pilot>)(?P<ship>)(?P<weapon>)'

        overviewSettings = settings.getOverviewSettings(self.character)
        pilotAndWeaponRegex = self.createOverviewRegex(
            overviewSettings) or basicPilotAndWeaponRegex

        self.damageOutRegex = re.compile(
            _logLanguageRegex[self.language]['damageOut'] +
            basicPilotAndWeaponRegex)

        self.damageInRegex = re.compile(
            _logLanguageRegex[self.language]['damageIn'] +
            basicPilotAndWeaponRegex)

        self.armorRepairedOutRegex = re.compile(
            _logLanguageRegex[self.language]['armorRepairedOut'] +
            pilotAndWeaponRegex)
        self.hullRepairedOutRegex = re.compile(
            _logLanguageRegex[self.language]['hullRepairedOut'] +
            pilotAndWeaponRegex)
        self.shieldBoostedOutRegex = re.compile(
            _logLanguageRegex[self.language]['shieldBoostedOut'] +
            pilotAndWeaponRegex)

        self.armorRepairedInRegex = re.compile(
            _logLanguageRegex[self.language]['armorRepairedIn'] +
            pilotAndWeaponRegex)
        self.hullRepairedInRegex = re.compile(
            _logLanguageRegex[self.language]['hullRepairedIn'] +
            pilotAndWeaponRegex)
        self.shieldBoostedInRegex = re.compile(
            _logLanguageRegex[self.language]['shieldBoostedIn'] +
            pilotAndWeaponRegex)

        self.capTransferedOutRegex = re.compile(
            _logLanguageRegex[self.language]['capTransferedOut'] +
            pilotAndWeaponRegex)

        self.capNeutralizedOutRegex = re.compile(
            _logLanguageRegex[self.language]['capNeutralizedOut'] +
            pilotAndWeaponRegex)
        self.nosRecievedRegex = re.compile(
            _logLanguageRegex[self.language]['nosRecieved'] +
            pilotAndWeaponRegex)

        self.capTransferedInRegex = re.compile(
            _logLanguageRegex[self.language]['capTransferedIn'] +
            pilotAndWeaponRegex)
        #add nos recieved to this group in readlog

        self.capNeutralizedInRegex = re.compile(
            _logLanguageRegex[self.language]['capNeutralizedIn'] +
            pilotAndWeaponRegex)
        self.nosTakenRegex = re.compile(
            _logLanguageRegex[self.language]['nosTaken'] + pilotAndWeaponRegex)
    ...
    def readLog(self, logData):
        ...
        logisticsOut.extend(
            self.extractValues(self.hullRepairedOutRegex, logData))
        logisticsOut.extend(
            self.extractValues(self.shieldBoostedOutRegex, logData))
        logisticsIn = self.extractValues(self.armorRepairedInRegex, logData)
        logisticsIn.extend(
            self.extractValues(self.hullRepairedInRegex, logData))
        logisticsIn.extend(
            self.extractValues(self.shieldBoostedInRegex, logData))
        ...
        capDamageDone = self.extractValues(self.capNeutralizedOutRegex,
                                           logData)
        capDamageDone.extend(self.extractValues(self.nosRecievedRegex,
                                                logData))
        capDamageRecieved = self.extractValues(self.capNeutralizedInRegex,
                                               logData)
        capDamageRecieved.extend(
            self.extractValues(self.nosTakenRegex, logData))
    ...
    def extractValues(self, regex, logData, mining=False):
                ...
                if amount != 0:
                    ...
                    if settings.getMiningM3Setting():
                        if _type in _oreVolume:
                            returnGroup['amount'] = int(
                                amount) * _oreVolume[_type]
                        else:
        ...
        for match in group:
            ...
            pilotName = match.group('default_pilot') or match.group(
                'pilot') or '?'
            shipType = match.group('ship') or match.group(
                'default_ship') or pilotName
            weaponType = match.group('default_weapon') or match.group(
                'weapon') or 'Unknown'
            if amount != 0:
                returnGroup = {}
                returnGroup['amount'] = int(amount)
...
class PlaybackLogReader(BaseLogReader):
    def __init__(self, logPath, mainWindow):
        ...
        try:
            ...
            messagebox.showerror(
                tr("Error"),
                tr("This doesn't appear to be a EVE log file.") + "\n" +
                tr("Please select a different file."))
        ...
        try:
            ...
            messagebox.showerror(
                tr("Error"),
                tr("This doesn't appear to be a EVE combat log.") + "\n" +
                tr("Please select a different file."))
            raise BadLogException("not character log")
        logging.info('Log language is ' + self.language)

        startTimeRegex = _logLanguageRegex[self.language]['sessionTime']
        self.startTimeLog = datetime.datetime.strptime(
            re.search(startTimeRegex, self.log.readline()).group(0),
            "%Y.%m.%d %X")
        ...
        while (self.logLine ==
            ...
            self.log.readline()
            collisionCharacter, language = ProcessCharacterLine(
                self.log.readline())
        ...
        self.nextTime = datetime.datetime.strptime(
            self.timeRegex.findall(self.nextLine)[0], "[ %Y.%m.%d %X ]")
        ...
        while (line != ''):
            ...
            try:
                ...
                continue
            self.endTimeLog = datetime.datetime.strptime(
                nextTimeString, "[ %Y.%m.%d %X ]")
        endOfLog.close()

        endOfLog = open(logPath, 'r', encoding="utf8")
        line = endOfLog.readline()
        self.logEntryFrequency = [0] * (self.endTimeLog -
                                        self.startTimeLog).seconds
        while (line != ''):
            line = endOfLog.readline()
            try:
                nextTimeString = self.timeRegex.findall(line)[0]
                entryTime = datetime.datetime.strptime(nextTimeString,
                                                       "[ %Y.%m.%d %X ]")
                self.logEntryFrequency[(entryTime -
                                        self.startTimeLog).seconds] += 1
            except IndexError:
                continue
        endOfLog.close()
    ...
    def newStartTime(self, newTime):
        ...
        while (self.nextTime < newTime):
            ...
            try:
                ...
                continue
            self.nextTime = datetime.datetime.strptime(nextTimeString,
                                                       "[ %Y.%m.%d %X ]")
            self.nextLine = line
    ...
    def readLog(self):
        ...
        self.mainWindow.playbackFrame.timeSlider.set(
            (logReaderTime - self.startTimeLog).seconds)
...
class LogReader(BaseLogReader):
    def __init__(self, logPath, mainWindow):
        ...
        if (self.logLine ==
            ...
            collisionCharacter, language = ProcessCharacterLine(
                self.log.readline())
            logging.error('Log file collision on characters' + self.character +
                          " and " + collisionCharacter)
            messagebox.showerror(
                tr("Error"),
                tr("Log file collision on characters:") + "\n\n" +
                tr("{0} and {1}".format(self.character, collisionCharacter)) +"\n\n"+
                tr("This happens when both characters log in at exactly the same second."
                   ) +"\n"+
