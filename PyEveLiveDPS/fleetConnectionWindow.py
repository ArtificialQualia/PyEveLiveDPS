...
class FleetWindow(tk.Toplevel):
    def __init__(self, mainWindow):
        ...
        if len(characterEntries) == 0:
            ...
            tk.messagebox.showerror(
                tr("Error"),
                tr("PELD must be tracking a character before enabling fleet mode."
                   ))
            return
        ...
        self.wm_title(tr("PyEveLiveDPS Fleet Mode"))
        try:
        ...
        tk.Label(characterFrame,
                 text=tr("Character to use for fleet mode: ")).grid(row="0",
                                                                    column="1",
                                                                    sticky="w")
        ...
        characterFrame.grid(row=self.counter,
                            column="1",
                            columnspan="2",
                            sticky="w")
        nameDescription.grid(row=self.counter + 1,
                             column="1",
                             columnspan="2",
                             sticky="w")
        tk.Frame(self, height="15", width="10").grid(row=self.counter + 2,
                                                     column="1",
                                                     columnspan="2")
        self.counter += 3
        ...
        self.addEntrySetting(
            self.serverVar, tr("Server to use: "),
            tr("Don't change this unless your FC tells you to"))

        self.modeVar = tk.IntVar()
        self.modeVar.set(1)
        tk.Radiobutton(self,
                       text=tr("Fleet Member"),
                       variable=self.modeVar,
                       value=1).grid(row=self.counter, column="1", sticky="w")
        self.counter += 1
        tk.Radiobutton(self,
                       text=tr("FC (requires fleet boss)"),
                       variable=self.modeVar,
                       value=2).grid(row=self.counter, column="1", sticky="w")
        self.counter += 1
        tk.Frame(self, height="10", width="10").grid(row=self.counter,
                                                     column="1",
                                                     columnspan="5")
        self.counter += 1
        ...
        lowCPUCheckbutton = tk.Checkbutton(self,
                                           text=tr("Use low CPU mode"),
                                           variable=self.lowCPUVar)
        lowCPUCheckbutton.grid(row=self.counter,
                               column="1",
                               columnspan="2",
                               sticky="w")
        descriptor = tk.Label(
            self,
            text=tr(
                "Most features will be disabled, but networking will work normally"
            ))
        font = tkFont.Font(font=descriptor['font'])
        font.config(slant='italic')
        descriptor['font'] = font
        descriptor.grid(row=self.counter + 1,
                        column="1",
                        columnspan="2",
                        sticky="w")
        tk.Frame(self, height="10", width="10").grid(row=self.counter + 2,
                                                     column="1",
                                                     columnspan="2")
        self.counter += 3

        tk.Frame(self, height="10", width="1").grid(row="99",
                                                    column="1",
                                                    columnspan="5")
        ...
        self.mainWindow.mainMenu.menu.insert_command(
            5, label=tr("End Fleet Mode"), command=self.logout)
    ...
    def login(self):
        ...
        self.sockMgr = socketManager.SocketManager(self.serverVar.get(),
                                                   self.characterName,
                                                   requestArgs,
                                                   self.loginNotificationQueue)
        self.sockMgr.start()
        self.loginButton.configure(state=tk.DISABLED)
        self.loginWindowThread = threading.Thread(target=self.waitForLogin,
                                                  daemon=True)
    ...
    def logout(self):
        if hasattr(self,
                   'loginWindowThread') and self.loginWindowThread.is_alive():
        ...
        self.mainWindow.mainMenu.menu.insert_command(
            5,
            label=tr("Fleet Mode"),
            command=lambda: FleetWindow(self.mainWindow))
    ...
    def addEntrySetting(self, var, labelText, descriptorText):
        ...
        centerFrame.grid(row=self.counter,
                         column="1",
                         columnspan="2",
                         sticky="w")
...
class SocketNotificationWindow(tk.Toplevel):
    def __init__(self, loginQueue):
        ...
        self.wm_attributes("-topmost", True)
        self.wm_title(tr("PyEveLiveDPS Awaiting Login"))
        try:
            self.iconbitmap(sys._MEIPASS + '\\app.ico')
        except Exception:
            try:
        ...
        tk.Label(self, text=tr('Waiting for you to login...')).grid(row=1,
                                                                    column=1)
