...
class DPSGraph(tk.Frame):
    def __init__(self, parent, **kwargs):
        ...
        self.graphFigure.subplots_adjust(left=(30 / 100),
                                         bottom=(15 / 100),
                                         right=1,
                                         top=(1 - 15 / 100),
                                         wspace=0,
        ...
        # self.canvas.show()
        self.canvas.draw()
        # see https://github.com/jarvisteach/appJar/issues/551
    ...
    def readjust(self, highestAverage):
        ...
        if (highestAverage < 900):
            self.graphFigure.subplots_adjust(left=(33 / self.windowWidth),
                                             top=(1 - 15 / self.windowWidth),
                                             bottom=(15 / self.windowWidth),
                                             wspace=0,
                                             hspace=0)
        elif (highestAverage < 9000):
            self.graphFigure.subplots_adjust(left=(44 / self.windowWidth),
                                             top=(1 - 15 / self.windowWidth),
                                             bottom=(15 / self.windowWidth),
                                             wspace=0,
                                             hspace=0)
        elif (highestAverage < 90000):
            self.graphFigure.subplots_adjust(left=(55 / self.windowWidth),
                                             top=(1 - 15 / self.windowWidth),
                                             bottom=(15 / self.windowWidth),
                                             wspace=0,
                                             hspace=0)
        else:
            self.graphFigure.subplots_adjust(left=(66 / self.windowWidth),
                                             top=(1 - 15 / self.windowWidth),
                                             bottom=(15 / self.windowWidth),
                                             wspace=0,
                                             hspace=0)
        if (highestAverage < 100):
            ...
            self.graphFigure.axes[0].set_ylim(bottom=0,
                                              top=(highestAverage +
                                                   highestAverage * 0.1))
        self.graphFigure.axes[0].get_yaxis().grid(True,
                                                  linestyle="-",
                                                  color="grey",
                                                  alpha=0.2)
        self.canvas.draw()
    ...
    def animateLine(self, yValues, categories, lines, zorder):
                    ...
                    if value >= lineCategory["transitionValue"]:
                        if lineCategoryTracker == categoryIndex:
                            ...
                            if (lineNumber < len(lines)):
                                lines[lineNumber].set_data(
                                    range(index - len(currentLine), index),
                                    currentLine)
                                lines[lineNumber].set_color(
                                    categories[lineCategoryTracker]["color"])
                            else:
                                newLine, = self.subplot.plot(
                                    range(index - len(currentLine), index),
                                    currentLine,
                                    categories[lineCategoryTracker]["color"],
                                    zorder=zorder)
                            ...
                            currentLine.append(value)
                elif value >= lineCategory[
                        "transitionValue"] and value < categories[
                            categoryIndex + 1]["transitionValue"]:
                    if lineCategoryTracker == categoryIndex:
                        currentLine.append(value)
                    else:
                        if (lineNumber < len(lines)):
        ...
        if (lineNumber < len(lines)):
            lines[lineNumber].set_data(
                range(len(smoothed) - len(currentLine), len(smoothed)),
                currentLine)
            lines[lineNumber].set_color(
                categories[lineCategoryTracker]["color"])
        else:
    ...
    def smoothListGaussian(self, list, degree=5):
        ...
        for i in range(len(smoothed)):
            smoothed[i] = sum(
                np.array(list[i:i + window]) * weight) / sum(weight)
