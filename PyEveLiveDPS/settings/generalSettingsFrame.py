...
class GeneralSettingsFrame(tk.Frame):
    def __init__(self, parent, mainWindow, **kwargs):
        ...
        self.addSetting(
            self.secondsVar, tr("Number of seconds to average values:"),
            tr("Recommended to set this value higher than your weapon cycle time"
               ))
        ...
        self.addSetting(
            self.intervalVar,
            tr("How often to update graph/labels in milliseconds:"),
            tr("The lower you set this value, the higher your CPU usage will be"
               ))

        self.transparencyVar = tk.StringVar()
        self.transparencyVar.set(settings.getCompactTransparency())
        self.addSetting(self.transparencyVar,
                        tr("Window transparency percentage in compact mode:"),
                        tr("100 is fully visible, 0 is invisible"))
    ...
    def doSettings(self):
        try:
            ...
            tk.messagebox.showerror(
                tr("Error"),
                tr("Please enter a number for number of seconds to average DPS"
                   ))
            return
        if (secondsSetting < 2 or secondsSetting > 600):
            tk.messagebox.showerror(
                tr("Error"),
                tr("Please enter a value between 2-600 for number of seconds to average DPS"
                   ))
            return
        ...
        try:
            ...
            tk.messagebox.showerror(
                tr("Error"),
                tr("Please enter a number for milliseconds to update graph"))
            return
        if (intervalSetting < 10 or intervalSetting > 1000):
            tk.messagebox.showerror(
                tr("Error"),
                tr("Please enter a value between 10-1000 for milliseconds to update graph"
                   ))
            return
        ...
        if (intervalSetting < 50):
            okCancel = tk.messagebox.askokcancel(
                tr("Continue?"),
                tr("Setting the graph update interval to less than 50ms is generally a bad idea.  Your CPU won't like it."
                   ) + tr("Would you like to keep these settings?"))
            if not okCancel:
                return
        ...
        try:
            ...
            tk.messagebox.showerror(
                tr("Error"),
                tr("Please enter a number for compact transparency percentage")
            )
            return
        if (compactTransparencySetting < 1
                or compactTransparencySetting > 100):
            tk.messagebox.showerror(
                tr("Error"),
                tr("Please enter a value between 1-100 for compact transparency percentage"
                   ))
            return
