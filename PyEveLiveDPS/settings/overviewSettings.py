    ...
    def _animate(self):
        try:
            self.gif = tk.PhotoImage(
                file=self.gif_file, format='gif -index {}'.format(
                    self._num))  # Looping through the frames
...
class OverviewNotification(tk.Toplevel):
    def __init__(self):
        ...
        self.wm_title(tr("PyEveLiveDPS Overview Settings Warning"))
        try:
        ...
        tk.Label(self, image="::tk::icons::warning").grid(column="0",
                                                          ...
                                                          rowspan="90",
                                                          padx="15")
        tk.Label(self, text=warningText, anchor='w',
                 justify=tk.LEFT).grid(column="1", row="0", sticky="w")
...
class OverviewSettingsWindow(tk.Toplevel):
    def __init__(self):
        ...
        self.wm_attributes("-topmost", True)
        self.wm_title(tr("PyEveLiveDPS Overview Settings"))
        try:
            self.iconbitmap(sys._MEIPASS + '\\app.ico')
        except Exception:
            try:
        ...
        if (platform.system() == "Windows"):
            ...
            self.overviewPath = oShell.SpecialFolders(
                "MyDocuments") + "\\EVE\\Overview\\"
        else:
        ...
        topLabel = tk.Label(
            canvasFrame,
            text=tr(
                "You need to export your EVE overview settings for PELD to use them:"
            ))
        topLabel.grid(row="0", column="1")

        try:
            self.image = AnimatedGif(
                canvasFrame,
                sys._MEIPASS + '\\images\\peld-overview-export.gif')
            self.image.grid(row="1", column="1")
            self.image.start()
        except Exception:
            try:
                path = os.path.join('PyEveLiveDPS', 'images',
                                    'peld-overview-export.gif')
        ...
        if self.overviewFiles == {}:
            ...
            self.addSetting(settingsFrame, tr('default'), default=True)
        else:
            if 'default' in self.overviewFiles:
                self.addSetting(settingsFrame, tr('default'), default=True)
            for character in self.overviewFiles:
                if character != tr("default"):
    ...
    def addSetting(self, settingsFrame, characterName, default=None):
        ...
        settingLabel = tk.Label(innerFrame,
                                text=characterName,
                                padx=0,
                                borderwidth=0)
        ...
        if default:
            settingDescriptor = tk.Label(
                innerFrame,
                text=tr(
                    "(this is the overview setting applied to new characters)")
            )
        ...
        openOverviewFile = lambda: self.processOverviewFile(
            characterName, overviewLabel,
            tk.filedialog.askopenfilename(initialdir=self.overviewPath,
                                          title=tr("Select overview file")))
        openOverviewButton = tk.Button(
            buttonFrame,
            text=tr(" Select overview settings file "),
            command=openOverviewFile)
        openOverviewButton.grid(row="0", column="0")
        tk.Frame(buttonFrame, height="1", width="15").grid(row="0", column="1")
        revertEVEDefaultFunc = lambda: self.revertEVEDefault(
            characterName, overviewLabel)
        revertEVEDefaultButton = tk.Button(
            buttonFrame,
            text=tr(" Use default EVE overview settings "),
            command=revertEVEDefaultFunc)
        revertEVEDefaultButton.grid(row="0", column="2")
        if not default:
            tk.Frame(buttonFrame, height="1", width="15").grid(row="0",
                                                               column="3")
            revertDefaultFunc = lambda: self.revertPELDDefault(
                characterName, overviewLabel)
            revertDefaultButton = tk.Button(
                buttonFrame,
                text=tr(" Use PELD default overview setting "),
                command=revertDefaultFunc)
