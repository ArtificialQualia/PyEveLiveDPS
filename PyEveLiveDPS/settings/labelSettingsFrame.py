...
class LabelSettingsFrame(tk.Frame):
    ...
    def makeGrids(self):
        ...
        self.gridListLeft = [[
            self.makeGridBlock(self.gridFrameLeft, row, i) for row in range(8)
        ] for i in range(self.gridColumns[0])]
        self.gridFrameLeft.grid(row="5", column="1", padx="10")

        self.gridFrameRight = tk.Frame(self)
        self.gridListRight = [[
            self.makeGridBlock(self.gridFrameRight, row, i) for row in range(8)
        ] for i in range(self.gridColumns[1])]
        ...
        for item, entries in self.labels.items():
            ...
            try:
                GridEntry(self.gridListLeft[column][row], title,
                          entries["decimalPlaces"], entries["inThousands"])
            except IndexError:
                column = column - len(self.gridListLeft)
                GridEntry(self.gridListRight[column][row], title,
                          entries["decimalPlaces"], entries["inThousands"])
                    ...
                    if len(gridBoxChildren) > 0:
                        ...
                        for name, text in self.text.items():
                            if text == gridEntry.getLabelText():
                                ...
                                self.labels[name][
                                    "column"] = column + columnAdder
                                self.labels[name][
                                    "decimalPlaces"] = gridEntry.getListboxValue(
                                    )
                                self.labels[name][
                                    "inThousands"] = gridEntry.getCheckboxValue(
                                    )
    ...
    def moveGridEntry(self, title, decimalPlaces, inThousands, gridBox,
                      parent):
        oldBox = self._nametowidget(parent.winfo_parent())
        if len(gridBox.winfo_children()) > 0 and not gridBox.winfo_children(
        )[0] == parent:
            #this means we need to swap boxes
            gridEntry = gridBox.winfo_children()[0]
            parent.destroy()
            GridEntry(oldBox, gridEntry.getLabelText(),
                      gridEntry.getListboxValue(),
                      gridEntry.getCheckboxValue())
            gridEntry.destroy()
        else:
            parent.destroy()
...
class GridEntry(tk.Frame):
    def __init__(self,
        ...
        self.listbox = listbox = tk.Spinbox(self,
                                            from_=0,
                                            to=9,
                                            width=1,
                                            borderwidth=1,
        ...
        self.checkbox = checkbox = tk.Checkbutton(self,
                                                  text="K",
        ...
        singleLabel.bind("<Button-1>",
                         lambda e: self.dragStart(e, listbox, checkbox))
    ...
    def dragStart(self, event, listbox, checkbox):
        ...
        event.widget.bind("<ButtonRelease-1>",
                          lambda e: self.dragStop(e, listbox, checkbox))
    ...
    def dragStop(self, event, listbox, checkbox):
        ...
        for gridSection in [
                self.parent.gridListLeft, self.parent.gridListRight
        ]:
            for gridRow in gridSection:
                for gridBox in gridRow:
                    if (pointerx >= gridBox.winfo_rootx() and pointerx <=
                            gridBox.winfo_rootx() + gridBox.winfo_width()
                            and pointery >= gridBox.winfo_rooty() and pointery
                            <= gridBox.winfo_rooty() + gridBox.winfo_height()):
                        parentName = event.widget.winfo_parent()
                        parent = event.widget._nametowidget(parentName)
                        self.parent.moveGridEntry(
                            title=event.widget["text"],
                            decimalPlaces=listbox.get(),
                            inThousands=checkbox.var.get(),
                            gridBox=gridBox,
                            parent=parent)
                        return
        event.widget.grid()
...
class FloatingWindow(tk.Toplevel):
    def __init__(self, gridEntry=None, width=100, *args, **kwargs):
        ...
        GridEntry(self, gridEntry.getLabelText(), gridEntry.getListboxValue(),
                  gridEntry.getCheckboxValue()).grid(row="0", column="0")
