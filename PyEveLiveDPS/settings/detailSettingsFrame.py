...
class DetailSettingsFrame(tk.Frame):
    ...
    def __init__(self, parent, mainWindow, **kwargs):
        ...
        self.windowDisabled = tk.Checkbutton(
            self,
            text=tr("Show Pilot Breakdown window"),
            variable=checkboxValue)
        self.windowDisabled.var = checkboxValue
        self.windowDisabled.grid(row="1", column="1", columnspan="2")

        tk.Frame(self, height="20", width="10").grid(row="2",
                                                     column="1",
                                                     columnspan="2")
    ...
    def makeListBox(self):
        tk.Label(
            self,
            text=tr("Order priority for pilots and weapons in the breakdown"
                    )).grid(row="3", column="1")
        descriptor = tk.Label(self,
                              text=tr("drag and drop entries to order them"))
    ...
    def doSettings(self):
        ...
        return {
            "detailsWindowShow": self.windowDisabled.var.get(),
