...
class MainWindow(tk.Tk):
    ...
    def addToTaskbar(self):
        ...
        try:
            if (self.platform == "Windows"):
                ...
                try:
                    res = windll.user32.SetWindowLongPtrW(
                        hwnd, GWL_EXSTYLE, style)
                except AttributeError:
                    res = windll.user32.SetWindowLongW(hwnd, GWL_EXSTYLE,
                                                       style)
            ...
            logging.exception(
                "Error adding PELD to Windows taskbar.  This should never happen, but execution can continue normally."
            )
            logging.exception(e)

    def addMenus(self):
        # character menu options are added dynamically by CharacterDetector, so we pass this into that
        self.characterMenu = tk.Menubutton(text=tr("Character..."),
                                           background="black",
                                           fg="white",
                                           borderwidth="1",
        ...
        self.mainMenu = tk.Menubutton(text=tr("File..."),
                                      background="black",
                                      fg="white",
                                      borderwidth="1",
                                      highlightbackground="black",
        ...
        self.mainMenu.menu.add_command(
            label=tr("Edit Profile Settings"),
        ...
        self.mainMenu.menu.add_cascade(label=tr("Profile"),
                                       menu=self.profileMenu)
        self.mainMenu.menu.add_separator()
        self.mainMenu.menu.add_command(label=tr("Clear Total/Peak Values"),
                                       state=tk.DISABLED)
        self.mainMenu.menu.add_separator()
        self.mainMenu.menu.add_command(
            label=tr("Fleet Mode"),
        ...
        getLogFilePath = lambda: tk.filedialog.askopenfilename(
            initialdir=self.characterDetector.path, title="Select log file")
        self.mainMenu.menu.add_command(label=tr("Playback Log"),
                                       command=lambda: self.characterDetector.
                                       playbackLog(getLogFilePath()))
        self.mainMenu.menu.add_separator()
        self.mainMenu.menu.add_command(label=tr("Quit"),
                                       command=self.quitEvent)
    ...
    def addQuitButton(self):
        ...
        self.quitButton = tk.Canvas(width=15,
                                    height=15,
                                    background="black",
    ...
    def addCollapseButton(self, parent, row, column):
        ...
        self.collapseButton = tk.Canvas(parent,
                                        width=15,
                                        height=15,
                                        background="black",
                                        highlightbackground="white",
    ...
    def addMinimizeButton(self, parent, row, column):
        ...
        self.minimizeButton = tk.Canvas(parent,
                                        width=15,
                                        height=15,
                                        background="black",
                                        highlightbackground="white",
    ...
    def minimizeEvent(self, event):
        ...
        if settings.fleetWindowShow and hasattr(
                self, 'fleetWindow') and self.animator.dataQueue:
    ...
    def addPlaybackFrame(self, startTime, endTime):
        ...
        self.mainMenu.menu.insert_command(
            8,
            label=tr("Stop Log Playback"),
            command=self.characterDetector.stopPlayback)
        self.topLabel.configure(text=tr("Playback Mode"))
        self.topLabel.grid()
        self.playbackFrame = playbackFrame.PlaybackFrame(
            self, startTime, endTime)
        self.playbackFrame.grid(row="11",
                                column="1",
                                columnspan="19",
                                sticky="news")
    ...
    def removePlaybackFrame(self):
        ...
        self.mainMenu.menu.insert_command(
            8,
            label=tr("Playback Log"),
            command=lambda: self.characterDetector.playbackLog(getLogFilePath(
            )))
        self.topLabel.grid_remove()
    ...
    def quitEvent(self, event=None):
        ...
        if not event or (event.x >= 0 and event.x <= 16 and event.y >= 0
                         and event.y <= 16):
            logging.info(
                'quit event received, saving window geometry and stopping threads'
            )
