    ...
    def initializeLabels(self):
        for index in self.labels:
            self.labels[index]["label"] = Label(
                ...
                settings=self.labelSettings[index],
                background="black")
            textLabel = tk.Label(self.labels[index]["label"],
                                 text=self.labels[index]["text"],
                                 fg="white",
                                 background="black")
            ...
            self.labels[index]["label"].grid(
                row=self.labelSettings[index]["row"],
                column=column,
                sticky="n")
...
class Label(tk.Frame):
    def __init__(self, parent, text, settings, **kwargs):
        ...
        tk.Label(self, text=text, fg="white",
                 background="black").grid(row="0", column="1")
        self.numberLabel = tk.Label(self,
                                    text="0.0",
                                    fg="white",
                                    background="black")
        ...
        self.showPeak = False
        self.peakValue = 0.0
        self.peakLabel = tk.Label(self,
                                  text=tr("Peak") + ":",
                                  fg="white",
                                  background="black")
        self.peakLabel.grid(row="1", column="1")
        self.peakLabel.grid_remove()
        self.peakNumberLabel = tk.Label(self,
                                        text="0.0",
                                        fg="white",
                                        background="black")
        ...
        self.totalNumberLabel = tk.Label(self,
                                         text="0.0",
                                         fg="white",
                                         background="black")
    ...
    def updateTotal(self, number):
        ...
        color = self.findColor(self.totalValue)
        self.totalNumberLabel["text"] = self.convertNumberToStr(
            self.totalValue)
        self.totalNumberLabel.configure(fg=color)
